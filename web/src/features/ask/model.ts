 ...
 * 무엇을 보고 답할 것인가.
 *
 * 셋은 성격이 다르다. **문서·룰**은 사람이 써 놓은 글이고, **설계 데이터**는 이미 저장돼
 * 이 앱에 들어와 있는 판이며, **라이브 디자인**은 지금 설계자의 CAD 에 떠 있는 —
 * 아직 저장조차 되지 않았을 수 있는 — 판이다.
 *
 * 셋을 한 자리에 둔 이유는 물음이 같기 때문이다. "이 선폭 괜찮나?"라는 물음에 문서는
 * 규정을 답하고, 저장된 판은 전례를 답하고, 라이브는 지금 이 순간을 답한다.
 */
export type Source = "docs" | "design" | "live";
