 ...
 * 읽고 나온 것으로 믿는다.
 *
 * 예행으로 붙었으면 값은 읽어 준다. 다만 그 값이 설계 툴이 아니라 이미 들어와 있는 판에서
 * 온 것이라는 사실을 답 안에 적는다. 설계 툴을 읽었다고 믿게 두면 그 답은 거짓이 된다.
 */
function fromLive(question: string, board: Board | null): { text: string; cites: Cite[] } {
  if (!board) {
    return {
      text:
        ...
        "붙고 나면 설계 툴에 지금 열려 있는 판을 그대로 읽습니다. " +
        "오른쪽에서 연결하고 열린 판을 고르세요.",
      cites: [],
    };
  }
  ...
  return {
    text:
      `${base.text}\n\n` +
      "이 값은 설계 툴이 아니라 이미 들어와 있는 판에서 읽은 것입니다 — MCP 가 붙기 " +
      "전까지의 예행입니다.",
    cites: base.cites,
  };
}
