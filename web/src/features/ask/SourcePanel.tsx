                ...
                {live.designs.map((d) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <b>{d.name}</b>
                      {/* 설계 툴에서 고쳐 놓고 아직 저장하지 않은 판. CAD 창 제목에 붙는
                          별표와 같은 뜻이다.

                          이 표가 붙은 판만 라이브와 설계 데이터의 답이 갈린다 — 표가 없는
                          판은 어느 쪽으로 물어도 같은 답이 나온다. 그래서 이 표는 장식이
                          아니라 "여기서만 라이브가 쓸모 있다"는 안내다. */}
                      {d.dirty && (
                        <span className={s.liveDirty} title="설계 툴에서 고친 뒤 아직 저장하지 않았습니다">
                          수정됨
                        </span>
                      )}
                    </button>
